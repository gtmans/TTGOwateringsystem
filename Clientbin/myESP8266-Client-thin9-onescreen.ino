...
int numscreens  = 1;                // number of info screens to display -1 - extra 2 temp screens
...
void onescreen(){
    ...
    tft.setTextSize    (5); 
    tft.setCursor      (0,20);
    if (humidity1<=threshold){tft.setTextColor(TFT_RED,TFT_BLACK);}else{tft.setTextColor(TFT_GREEN,TFT_BLACK);}    
    tft.print          (humidity1);
    tft.print          ("%");
    
    tft.setCursor      (120,20);
    if (humidity2<=threshold){tft.setTextColor(TFT_RED,TFT_BLACK);}else{tft.setTextColor(TFT_GREEN,TFT_BLACK);}    
    tft.print          (humidity2);
    tft.print          ("%");
    ...
    tft.setCursor       (0,80);
    if (sensor1serverup){tft.setTextColor(TFT_GREEN,TFT_BLACK);tft.print("UP "); 
    } else {
      if (timesinces1<75) {tft.setTextColor(TFT_GREEN,TFT_BLACK);} else {tft.setTextColor(TFT_RED,TFT_BLACK);}
      tft.print(timesinces1);
    }
    tft.setTextColor(TFT_WHITE,TFT_BLACK);   

    tft.setCursor       (120,80);
    if (sensor2serverup){tft.setTextColor(TFT_GREEN,TFT_BLACK);tft.print("UP ");
    } else { 
      if (timesinces2<75) {tft.setTextColor(TFT_GREEN,TFT_BLACK);} else {tft.setTextColor(TFT_RED,TFT_BLACK);}
      tft.print(timesinces2);
      tft.setTextColor(TFT_WHITE,TFT_BLACK);   
    }
    tft.setTextSize    (2);       
