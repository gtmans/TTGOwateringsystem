/* com 16
 * \Arduino\LORAWAN\TTGO-LoRa-Series-master\LoRaReceiver\LoRaReceiver-12.73 to use with
 ...
 * reads text from lorasenders higrow (for T3 LoRa32 oled)
 */
//LilyGO TTGO T-Higrow LoRa Shield - 868MHz
#include <SPI.h>
#include <LoRa.h>
...
#include "EEPROM.h"
bool eeprom  = true;
bool espres  = true;             //restart ESP every 60 min to avoid hanging
bool debug   = false;
...
int    count  = 0;
int    expLEN = 20;         // length of "MYDATA01102500100000"
String expID  = "MYDATA";   // your chosen identifier
...
//long interval     = 600000;  // interval 10 minutes
long interval       = 3600000; // interval 60 minutes
  ...
  if (LoRa.parsePacket()) {
      ...
      if (lenIN==expLEN){
        ...
        if (ID==expID){
          ...
          // identifier send to sender 0? from receiver 1 received OK
          ...
          bool sndextramsg=true;
          if(sndextramsg){
            //SEND AGAIN 2 BE SURE        
            delay             (1000); 
            Serial.print      ("sending  :");   
            Serial.println    (myMessage);  
            LoRa.beginPacket  ();
...
void breakdown(String input){
  ...
  MN  = input.substring(15,20);//"00000";  // messagenumber
  T   = TT.toInt();                        // 25 io "025"
  TT  = String(T);
  H   = HH.toInt();                        // 99 io "099"
  HH  = String(H);
  S   = SN.toInt();                       
...
void ParseReadings(int tsensor,bool updmsg){
...
void displayOled(){
     display.clear();
     display.drawString  (0    ,0, SensTim [0]);
     display.drawString  (64   ,0, SensTim [1]);
     display.drawString  (0    ,16,Sensors [0]);
     display.drawString  (0    ,32,Sensors [1]);
     display.drawString  (0 +24,48,SensMes [0].substring(2,5));
     display.drawString  (64+24,48,SensMes [1].substring(2,5));             
     display.drawString  (0    ,48,String(NrMsgs[0]));
