 ...
 * to use with
 * \Arduino\LORAWAN\TTGO-LoRa-Series-master\LoRaReceiver-higrow-16.9
 ...
 */
#define USE_INSIDE  1         // SET! reduce radiosignal
...
#define SND_ERROR   1         // SET! send errors instead of sensor temperature

#include <SPI.h>
#include <LoRa.h>
#include "LoRa_ext_board_def.h"        // settings of T-Higrow LoRa Shield 
#include "EEPROM.h"
...
bool dhton        = false;    // SET! measure temperature from sensor
bool usesyncword  = false;    // SET! use same in receiver sketch

unsigned int  count = 1;
String  ID    = "MYDATA";     // SET! identifier
String  SN    = "01";         // SET! sensor number SET THIS ACCORDING TP SENSOR!!
...
int sleepmins = 61;             //#minutes 2 sleep 70max;
//int sleeptime = sleepmins * 60; // 1 mins * 60 seconds
int sleeptime = (sleepmins * 60)-30; // try to get close to 60 minutes
...
//EEPROM.writeInt       (0,count);// do not update counter when not suc6full 
  EEPROM.writeInt       (3*sizeof(int),errors);// save nr errors
  EEPROM.commit         (); 
  delay                 (200);
  esp_deep_sleep_start  ();  
}
